...
class SAG_Plugin {
    ...
    /** @var SAG_Plugin|null The single instance. */
    private static $instance = null;

    /** Private constructor prevents `new SAG_Plugin()` from outside. */
    ...
    private function register_hooks() {
        if ( class_exists( 'SAG_REST_API' ) ) {
            $rest = new SAG_REST_API();
            add_action( 'rest_api_init', array( $rest, 'register_routes' ) );
        }
        if ( class_exists( 'SAG_Settings' ) ) {
            $settings = new SAG_Settings();
            add_action( 'admin_init', array( $settings, 'register' ) );
        }
        if ( is_admin() && class_exists( 'SAG_Admin' ) ) {
            $admin = new SAG_Admin();
