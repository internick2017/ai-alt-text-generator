...
final class TestEndpointTest extends TestCase {
    ...
    public function test_test_connection_returns_error_when_no_api_key() {
        ...
        $result = ( new \INSAG_REST_API() )->test_connection( new \WP_REST_Request() );

        $this->assertInstanceOf( \WP_Error::class, $result );
        $this->assertSame( 'insag_no_api_key', $result->get_error_code() );
    }

    public function test_test_connection_returns_ok_on_success() {
        Functions\when( 'get_option' )->alias( function ( $key, $default = '' ) {
            return $key === 'insag_openai_api_key' ? 'sk-test' : 'gpt-4o-mini';
        } );
    ...
    public function test_test_connection_returns_error_on_openai_api_error() {
        Functions\when( 'get_option' )->alias( function ( $key, $default = '' ) {
            return $key === 'insag_openai_api_key' ? 'sk-bad' : 'gpt-4o-mini';
        } );
        Functions\when( 'wp_json_encode' )->alias( 'json_encode' );
        ...
        $this->assertInstanceOf( \WP_Error::class, $result );
        $this->assertSame( 'insag_test_failed', $result->get_error_code() );
